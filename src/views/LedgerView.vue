...
import { formatIsoDate, formatPeriod } from '@/util/date'
...
<template>
  ...
  <template v-if="ledgerStore.ledger">
    <div class="mb-5 text-3xl text-center">
      {{ formatPeriod(ledgerStore.ledger.period) }}
    </div>
    <div
      v-for="subLedger in ledgerStore.ledger?.subLedgers"
      :key="subLedger.accountId"
      class="mb-5"
    >
